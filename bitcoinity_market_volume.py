...
url = "https://data.bitcoinity.org/markets/volume/30d?c=e&t=b"
headers,data = get_headers_and_data(url)
proxies = get_proxy()
tablename = "bitcoininity_market_volume"
keys = 'Exchange,volume,market_share,graph_val'
#it will create table id not exist
...
for val in aggre_additional_info:
    ...
    for graph_hash in json_val["data"]:
        if graph_hash["key"] == exchange:
            graph_val = [{"time": get_convert_time(x[0]),"volume":round(x[1])} for x in graph_hash["values"]]
...
#this is the additional if from the site
